...
export class InOutLogPerDay {
    ...
    private calculateDurationSeconds(limitTime: number): number {
        return this.inOutLogs?.reduce((total, log) => {
...
export function groupLogsByDay(logs: InOutLogType[], limitTime: number): InOutLogPerDay[] {
    ...
    logs.forEach(log => {
        if (log.inTimeStamp && log.outTimeStamp) {
            const inDate = new Date(log.inTimeStamp * 1000).toISOString().split('T')[0];
